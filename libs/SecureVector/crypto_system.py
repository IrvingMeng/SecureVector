...
if args.genkey == 1:
    ...
    np.save(
        'libs/SecureVector/keys/privatekey_{}.npy'.format(args.key_size), [prikey])
    np.save(
        'libs/SecureVector/keys/publickey_{}.npy'.format(args.key_size), [pubkey])
    exit(1)
else:
    private_key = np.load(
        'libs/SecureVector/keys/privatekey_{}.npy'.format(args.key_size), allow_pickle=True)[0]
...
def calculate_sim(c_x, c_y, C_tilde_x, C_tilde_y, K, L, M):
    ...
    score = W_z * \
        sum([bar_c_xy[i]/(s_list[i] * np.e**((u_list[i]-2*L)/M))
            for i in range(K)])
    ...
    with open(pair_list, 'r') as f:
        for j in range(len(chunk_info_list)):
            ...
            for line in lines:
                ...
                # load files
                c_x, C_tilde_x = load_enrolled_file(
                    '{}/{}.npy'.format(folder, file1))
                c_y, C_tilde_y = load_enrolled_file(
                ...
                score, durations = calculate_sim(
                    c_x, c_y, C_tilde_x, C_tilde_y, K, L, M)
