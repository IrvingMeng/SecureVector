...
def distance_(embeddings0, embeddings1):
    ...
    norm = np.linalg.norm(embeddings0, axis=1) * \
...
def calculate_roc(thresholds, dist, actual_issame, nrof_folds=1):
    ...
    fold_idx = 0
    for threshold_idx, threshold in enumerate(thresholds):
        tprs[fold_idx, threshold_idx], fprs[fold_idx,
                                            threshold_idx], _ = calculate_accuracy(threshold, dist, actual_issame)
    _, _, accuracy[fold_idx] = calculate_accuracy(
        thresholds[best_threshold_index], dist, actual_issame)
